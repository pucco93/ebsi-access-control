...
This project was done for Distributed Systems course at the University of Urbino Carlo Bo, applied informatic faculty (Master's degree).

It aims to demonstrate the potential of Web3, it borrows a new European Decentralized method - which indentify legal entities and natural persons - to use these new **Decentralized Identities** (**DIDs**) in an **Access Control List** (**ACL**).
...
The project's scope is to define a new way of sharing resources between users who are identified by their unique EBSI DID, keeping centered the access policies defined by the resources.
...
This section goal is to explain better what is an EBSI DID why the European Union want to use them and how these are created.

To answer to the first question the EBSI DID as said above have been developed to identify users in a new manner having a cross-border ID that can be used by European countries for their citizens. Currently every country in EU has their digital ID which is centrified and not compatible between countries, in practical:

- The user has to request to an identity provider to receive his digital identity, this role is played in some countries by a government office while in others is delegated to official issuers (Italy is in the second countries list).
- Once the user has given all his personal data and contacted the issuer to confirm his identity (it can be a visit at his closest postal/government office or by a video-call), the issuer provide a digital identity to the requester.
- The user has now a digital identity which can be used online on the government's services.

At first sight this can be a good procedure, where a certified identity provider who is constantly in contact with the government has all the power on identifiy the user and give him access to the services, but in reality this can be unconvenient due to: providers full power to block a user, all personal data being shared to services instead of less possible data and the cross-border problem.

This last problem raises when the EU citizen wants to move to another country in EU, here the previous identity provider could be totally unofficial or the new country governament does not have access to the issuer database.
...
When the user send requests for official services (tax payments, new ID card, passport, etc) can send a request using his EBSI DID, on the other hand the office that receive the request can resolve the DID received with the request and see if the person is really who's claiming to be.
...
To create an EBSI DID the user must start by generating his DID and DID keys, which can be done using his public key, then the user can request a DID Document where there will be more information regarding how to verify his identity with its `verifiableMethods`.
...
However the European Commission does not grant the use of the blockchain to all the interested developers, because this is a permissioned blockchain, which means developers can test their dApps only after submitting a request and then receiving a positive response from EU responsible committee.
...
On the technical side it should be noted the tools used in the project:
...
- On the smart contracts side:
  ...
  - To create the key pair outside the dApp, it has been used `openssl` so that the user can use the same algorithm as Ethereum has.
        In a chapter below it will be explain exactly how to create the keys on the machine locally.
  - `Truffle suite` is a suite comprehending all the necessary tools to build, deploy and migrate smart contracts, this can also be used without Ganache and its UI, simply creating a virtual Testnet by command line.
  - `Ganache v7` is the tool used to create a personal blockchain and 10 accounts, these are used to deploy the smart contract and they can also be used to interact with the smart contract.
  - `Metamask` is used to import the web3 accounts used to interact with the smart contract, it also has many useful functions to retrieve data about the logged users or the smart contract.
- On the client side:
  ...
  - `@cef-ebsi/key-did-resolver` is a npm package which is open source and maintened by European Commission, it is used to create DIDs and to solve them and retrieve DID document. This can be used on natural persons or on legal entities. The two methods differ for the final result (natural persons starts with did:key:z while legal entities starts with did:ebsi).
  - `crypto` is a set of functions used to create and handle keys, in the project it is used to create the key pair in elliptic curve secp256k1 as made in Ethereum, these will be used to create the user DID and the user account.
...
One important note on this project is that it's always using the ECDSA algorithm to generate key pair.

For the most part of the development the most used IDE was Remix - Ethereum IDE, which can be found at this url [remix.ethereum.org](remix.ethereum.org).
This has some important features that the other IDEs don't have, like the compiler, the deploy and run transactions and debugger.
...
This informs the command line to create a newtowrk on `127.0.0.1` (localhost) on port: `7545` and identify the network with id `5777`, with a maximum gas cost of `30000000`.
...
- Ganache > v2.7.1 (or equivalent with London fork available)
...
- Build and deploy the smart contracts using command: `truffle migrate --network development` on a terminal from inside the repo, where the `truffle-config.json` is.
  - If there are any problems during this last operation it could be related to the gas limit which is caused by the size of the smart contract, to fix it consider to adjust gas limit by increasing it accordingly (this is normally defined by the command on the error logs).
...
7. Open the users view by clicking on the left menu item called 'Users' and then on 'Create users'
...
To have the same result but leaving the key pair generation to the dApp the user has to follow these steps:

1. Open the Users view by clicking on Users menu item on the left.
2. Click on Create user button on the right.
3. Now the user should see a dialog asking for some info, but the important one is the button to create a key pair. Clicking this will generate a key pair with the private key being displayed in the below text area.
...
In example `removeUser` was called `removeUsers` and was meant to delete an array of users looping through the passed array from client-side, but this has been changed to a function which changes only a user per time, this has downsides like running the same multiple time to emulate the deletion fo multiple users in one time, this also creates more requests to Metamask that needs to accept multiple times to complete the actions.

Some other functions were looping through the created items array before the refactor, to check if item was already in blockchain. Those have been optimized with the check into a mapping, differently from a for loop, this has less gas consumption during runtime and does not require to go through the entire array length.
...
Lastly the choice of using string type instead of bytes32 to limit length and smart contract size, this was caused by the ebsi did for a natural person being longer than 32 bytes.
...
- Users view has the button to create new users, this opens a dialog with required data and options to create the user. The table displays the ebsi did, a list of resources and some data regarding creation date and last access/update. In the users table the actions can remove a user or open a dialog to edit it, this last dialog gives the ability to add or remove resources from a user and change resources and roles to him.

- Resources view has a button to create a resource which requires only a name, the table displaying the name the users in blacklist and the actions. The available actions are delete and edit. Editing a resource means adding or removing users from its blacklist.

- Roles view shows the table with all the created roles, with each rows rendering the role name, a checkmark or cross icon to let the user know if it is custom or not and action to delete the role. This action is only available if the role is custom.
...
Sadly this problem raises because the project has to rely on ETH blockchain and then simulate the EBSI platform.
